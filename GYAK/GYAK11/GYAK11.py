...
#inputImg = np.random.randint(low=0.0, high=2.0, size=(5,5))
inputImg = np.random.randint(0,2,(5,5)).astype("float32")


# Hozz létre egy kernelt (numpy array-t)(3x3)
# Az értékei legyenek 0 vagy 1
# dtype legyen np.float32
kernel = np.random.randint(0,2,(3,3)).astype("float32")
...
Padding, Stride = 0,1
OutHeight = ((Padding*2 + ImgHeight)-(KernelHeight-Stride))
OutWidth = ((Padding*2 + ImgWidth)-(KernelWidth-Stride))
#print(OutHeight, OutWidth)

# Hozz létre egy az előbb kiszámolt kimeneti "kép"
# dimenziójával megegyező 0-kal feltöltött numpy array-t
outImg = np.zeros(shape=(OutHeight, OutWidth))  
#outImg
...
#layer = ConvolutionLayer(kernel_size=3, kernel_num=1)
#layer.patches_generator(inputImg)
    ...
    def patches_generator(self, image):
        ...
        for h in range(image_h-self.kernel_size+1):
            for w in range(image_w-self.kernel_size+1):
                #indexelj ki egy kernelnyi mátrixot a bemeneti képből a patchbe
                patch =  image[h:h+3, w:w+3]
...
for index in range(n):
    ...
    plt.imshow(outImg)
    features,kernels = conv.forward(outImg)
