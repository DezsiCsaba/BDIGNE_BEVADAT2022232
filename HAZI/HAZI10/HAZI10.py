...
def mnist_model() -> tf.keras.Sequential:
    model = tf.keras.Sequential([
        tf.keras.layers.Flatten(input_shape=(28, 28)),  #flattens to 28x28 = 784 pixels
        tf.keras.layers.Dense(128, activation='relu'),  #128 node=neuron
        tf.keras.layers.Dense(10, activation='softmax') #logits aray with length of 10
    ])
    return model
...
# %%
def model_evaluate(model, test_images, test_labels) -> float:
